	...
	<div class="card">
		<div class="card__top">
			<div class="card__title">
				<h3 class="card__title-text">Пакет "{{ data.title }}"</h3>
			</div>
			<p class="card__text">Включає:</p>
			<ul class="card__list">
				<li v-for="item in data.available" :key="item" class="card__list-item">
					<Icon name="check" class="card__list-icon" />
					<p class="card__list-text">{{ item }}</p>
				</li>
			</ul>
		</div>
		<div class="card__bottom">
			<div class="card__line"></div>
			<p class="card__title-text">{{ data.price }} грн / місяць</p>
			<button type="button" class="btn card__btn">
				<span class="btn__text btn__text--dark">Обрати</span>
			</button>
		</div>
	</div>
</template>
